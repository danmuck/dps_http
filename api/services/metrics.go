...
func MetricsHandler(svc *UserMetricsService) gin.HandlerFunc {
	...
	logs.Log("[api:users:metrics] initializing service handler [%s]", svc.String())
	return func(c *gin.Context) {
		svc.mu.Lock()
		total_users := svc.total_users
		total_roles := svc.total_roles
		users_over_time_points := utils.MapTimestampToInt64Points(svc.users_over_time)
		logs.Info("[api:users:metrics] total users: %d, total roles: %v", total_users, total_roles)
		if total_users == 0 {
			logs.Warn("[api:users:metrics] no users found, returning empty metrics")
			c.JSON(http.StatusOK, gin.H{
				"total_users":     0,
				"total_roles":     make(map[string]int64),
				"users_over_time": []utils.Point{},
		...
		logs.Debug("[api:users:metrics] service: %+v", svc)
...
func (svc *UserMetricsService) Register(router *gin.Engine) {
	logs.Log("[api:users:metrics] Register(%s)", svc.endpoint)
...
func (svc *UserMetricsService) Start() {
	...
	if err != nil {
		logs.Err("[api:users:metrics] failed to retrieve user count: %v", err)
		return
	}
	logs.Log("[api:users:metrics] loaded %d users", total_users)
	svc.total_users = total_users
	users_over_time, err := svc.GetBucket("metrics").ListItems()
	if err != nil {
		logs.Err("[api:users:metrics] failed to retrieve user metrics: %v", err)
		return
	}
	total_over_time := make(map[string]int64)
	for idx, raw := range users_over_time {
		if idx%250 == 1 {
			logs.Debug("[api:users:metrics] sanity check processing point %d/%d", idx, len(users_over_time))
		}
		timestamp, ok := raw["key"].(string)
		if !ok {
			logs.Warn("[api:users:metrics] found malformed user metrics point without timestamp: %v", raw)
			continue
		}
		count, ok := raw["value"].(int64)
		if !ok {
			logs.Warn("[api:users:metrics] found malformed user metrics point without count: %v", raw)
			continue
		}
		total_over_time[timestamp] = count
		if idx%500 == 1 {
			logs.Debug("[api:users:metrics] sanity check processing %d/%d { %s : %d }", idx, len(users_over_time), timestamp, count)
		}
	}
	total_roles, err := svc.UserCountByRole()
	if err != nil {
		logs.Err("[api:users:metrics] failed to retrieve user roles: %v", err)
		return
	}
	svc.mu.Lock()
	svc.running = true
	...
	logs.Info("[api:users:metrics] initialized with %d users, roles: %v, users_over_time points: %v",
...
func (svc *UserMetricsService) UserCountByRole() (map[string]int64, error) {
	logs.Log("[api:users:metrics] UserCountByRole()")
...
func backgroundService(svc *UserMetricsService) {
	if !svc.IsRunning() {
		logs.Warn("[api:users:metrics] is not running, exiting handler")
		return
	}

	logs.Log("[api:users:metrics] starting %s cycle", configs.METRICS_delay.String())
	defer svc.Stop()

	for svc.IsRunning() {
		logs.Log("[api:users:metrics] processing metrics...")
		total_users, err := svc.GetBucket("users").Count()
		if err != nil {
			logs.Err("[api:users:metrics] failed to retrieve user count: %v", err)
			return
		...
		for timestamp, users := range svc.users_over_time {
			go func(ts string, us int64) {
				if err := collection.Store(ts, us); err != nil {
					logs.Err("[api:users:metrics] failed to store user metrics: %v", err)
					return
				}
			}(timestamp, users)
		}
